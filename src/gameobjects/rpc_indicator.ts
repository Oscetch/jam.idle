...
export class RpcIndicator extends CenterRelativeGameObject {
  ...
  constructor(parent: GameObject) {
    super(
      ...
      new Point(Settings.PREFERED_WIDTH / 2, 50 + 100 + 100)
    );

    this.currentPoints = gameInformation.getRadiationPerClick();
  ...
  render(
    ...
    const availableRadiation = gameInformation.getRadiationPerClick();
  ...
  private getPointsImage(): TextImage {
    return new TextImage(
      `RPC: ${Math.floor(this.currentPoints).toString()}`,
      24,
      "#FFFFFF",
      700
    );
